...
/**
 * For very high dimensional functions (eg 5M evaluations of a 1000 dimension
 * function), re-seeding random generators, generating the random numbers and
 * mapping them into the right range on the fly is too slow, even for {@link RanQD1}.
 * <p> This companion class pre-generates the random numbers needed by {@link FractalFunction1D}
 * (in turn called by {@link FastFractal}) and stores them in lookup tables.
 * Changing the index simply becomes resetting the array index. <p> This results
 * in a small loss of statistical diversity for a considerable gain in speed,
 * even for quite large tables (eg 16K) since it is only done once for any given
 * function. <p> The user does not need to access this class directly, all
 * values are set by its owner, such as {@link FractalFunction1D}.
 *
